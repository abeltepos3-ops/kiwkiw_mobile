...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  // --- DUMMY USER ID SEMENTARA ---
  // Angka 1 ini kita pakai dulu biar kodingan jalan dan bisa nyambung database.
  // Nanti ini diubah ngambil dari data Session/Login.
  final int idUserYangLogin = 1; 
  ...
  // --- FUNGSI API KIRIM KE LARAVEL ---
  Future<void> _kirimDataKeDatabase() async {
    // PENTING: Cek CMD (ipconfig), pastikan IP laptop lu masih 192.168.1.15
    const String apiUrl = 'http://192.168.1.34:8000/api/mobile/checkout'; 
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        print('SUKSES: Data checkout masuk ke Laravel!');
      } else {
            ...
            child: ElevatedButton(
              onPressed: () async { 
                if (_currentStep == 1) {
                  ...
                  await _kirimDataKeDatabase();

                  // 2. CEK MOUNTED (Ini yang tadi kurang dan bikin error!)
                  if (!mounted) return;

                  // 3. Catat di order lokal
